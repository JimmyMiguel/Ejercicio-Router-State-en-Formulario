...
export function forlularioFun(): Element {
  class formularioCom extends HTMLElement {
    constructor() {
      super();
      this.render();
    }
    ...
    render() {
      ...
      `;
      const h2 = shadow.querySelector(".form-container > h2")
      const ultimaData = state.getState()
      h2==null ? console.log("error"):h2.textContent = ultimaData[0]
  ...
  // Definimos el custom element solo si no ha sido definido antes
  if (!customElements.get('form-tag')) {
    customElements.define('form-tag', formularioCom);
  }

  const formularioTag = document.createElement('form-tag');
  return formularioTag;
}
